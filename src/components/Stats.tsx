import fetchLastFmData from "../lib/fetchLastFmData";
import fetchHackaTimeStats from "../lib/fetchHackaTimeStats";
...
export default async function Stats() {
    ...
    const hackaTimeStats = await fetchHackaTimeStats();

    const projects = hackaTimeStats.data?.projects?.filter(x => x.name !== "chirag") || [];
        ...
        <div className="flex flex-col  min-w-[50%] max-w-full rounded-lg overflow-hidden shadow-lg m-4 md:m-12" style={{
            ...
            <div className="flex flex-col items-start justify-center p-4 md:p-6">
                ...
                <h1 className="text-3xl md:text-4xl 2xl:text-5xl font-bold text-white text-left items-center justify-center flex" style={saira.style}>
                    <Icon icon="catppuccin:vscode" className="mr-4" />
                    {hackaTimeStats.data?.human_readable_total || "0h 0m 😭"}
                </h1>

                <div className="flex flex-col items-start justify-center mt-4 ml-4">
                    <p className="text-sm md:text-md 2xl:text-lg text-gray-500" style={didactGothic.style}>
                        Worked most on: {projects?.[0]?.name || "None :/"}{(projects?.length || 0) > 0 ? ` (${projects?.[0]?.text})` : ""}
                    </p>
                    <p className="text-sm md:text-md 2xl:text-lg text-gray-500" style={didactGothic.style}>
                        Daily average: {hackaTimeStats.data?.human_readable_daily_average || "0h 0m 😭"}
                    </p>
                    <p className="text-sm md:text-md 2xl:text-lg text-gray-500" style={didactGothic.style}>
                        Most used language: {languages[0]?.name || "None :/"}
                    </p>
                </div>
            </div>
            ...
            <div className="px-4 pb-4 md:pb-6 md:px-6">
                <p className="text-sm md:text-md 2xl:text-lg text-gray-500 mb-1" style={didactGothic.style}>
                    {lastFmData.currentlyPlaying ? "Currently Playing" : "Last Listened to"}
                </p>
                <h2 className="text-xl md:text-2xl 2xl:text-3xl font-bold text-white" style={saira.style}>
                    {lastFmData.name || "Unknown Track"} - {lastFmData.artist || "Unknown Artist"}
                </h2>

                <div className="text-right w-full mt-6 justify-end items-end" style={didactGothic.style}>
                    <div className="flex items-end justify-end w-full">
                        <a href={lastFmData.url} target="_blank" rel="noopener noreferrer" className="flex flex-col items-end justify-end w-fit">
                            Open on
                            <svg xmlns="http://www.w3.org/2000/svg" width="5.87em" height="1.5em" viewBox="0 0 512 131">
                                <path d="M0 0h512v131H0z" fill="none" />
